...
constructor(
    ...
    override fun index(context: Context, withCache: Boolean, scope: CoroutineScope) =
        scope.launch {
            val result =
                try {
                    if (!withCache) context.toast("Reindexing")
                    val start = System.currentTimeMillis()
                    val library = indexImpl(context, withCache, this)
                    logD(
                        "Music indexing completed successfully in " +
                            "${System.currentTimeMillis() - start}ms")
                    if (!withCache) context.toast("Finished Indexing")
    ...
    private suspend inline fun Context.toast(msg: String) =
        withContext(Dispatchers.Main) { Toast.makeText(this@toast, msg, Toast.LENGTH_LONG).show() }
